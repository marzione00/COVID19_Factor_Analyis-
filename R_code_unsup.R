...
heatmap(x = cor(Dati_Covid[,5:11]), col = palette, symm = TRUE, margins = c(10, 10), main = 'Province TOT',dist(Dati_Covid[,5:11],method = 'euclidean'))
...
reg_full.pca<-prcomp(Dati_Covid_reg[,2:13], center = TRUE,scale. = TRUE)
...
corrplot(cor(Dati_Covid_reg[,2:13]))
palette = colorRampPalette(c("green", "blue", "red")) (20)
heatmap(x = cor(Dati_Covid_reg[,2:12]), col = palette, symm = TRUE, margins = c(10, 10),main = 'Regioni Tot', dist(Dati_Covid_reg[,2:12],method = 'euclidean'))
dend.dat<-dist(Dati_Covid_reg[,2:4] )
...
world.pca<-prcomp(Covid_mond[,6:15], center = TRUE,scale. = TRUE)
...
autoplot(peppo4,loadings = TRUE,loadings.size= 4, loadings.colour = 'blue',
         loadings.label = TRUE, loadings.label.size = 5,label=FALSE,label.vjust=1.5,loadings.label.repel=T,label.repel=T)+ggtitle('Dati Mondiali') +theme_bw()+theme(plot.title = element_text(hjust = 0.5)) 
summary(world.pca)
corrplot(cor(Covid_mond[,6:15]))
palette = colorRampPalette(c("green", "blue", "red")) (20)
heatmap(x = cor(Covid_mond[,6:15]), col = palette, symm = TRUE, margins = c(10, 10),main = 'Mondo',dist(Covid_mond[,6:15],method = 'euclidean'))
